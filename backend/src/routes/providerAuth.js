...
router.post("/login", async (req, res) => {
  ...
  const allowed = await rateLimitAllow(`provider-login:${req.ip}`, 10, 60);
  if (!allowed) return res.status(429).json({ error: "Too many login attempts. Try again in a minute." });

  const { email, password } = parsed.data;
  const account = await repo.findProviderUserByEmail(email);
  if (!account) return res.status(401).json({ error: "Incorrect email or password" });

  const valid = await bcrypt.compare(password, account.password_hash);
  if (!valid) return res.status(401).json({ error: "Incorrect email or password" });

  const claims = { role: "provider", providerId: account.provider_id };
  res.json({
    account: { id: account.id, name: account.name, email: account.email, providerId: account.provider_id, providerName: account.provider_name },
